...
function renderFoodRow(item) {
    ...
    const expDate = formatDateLocal(item.expDate);
    const regDate = formatDateLocal(item.regDate);
...
function renderFoodRegister(container, appState) {
    const categories = appState.foodCategories || [];
    const initialName = registerInitialName || '';

    container.innerHTML = `
        <button class="btn-outline mb-24" id="btn-back-food">＜ 戻る</button>
        <div class="form-group autocomplete-wrapper">
            <label for="input-food-name">品名</label>
            <input type="text" id="input-food-name" value="${escapeHTML(initialName)}" placeholder="例: 牛乳" autocomplete="off">
            <!-- カスタムサジェスト表示用の枠 -->
            <div id="food-autocomplete-list" class="autocomplete-list" style="display: none;"></div>
        </div>
        <div class="form-group">
            <label>数量</label>
            <input type="number" id="input-food-qty" value="0" min="0">
        </div>
        <div class="form-group">
            <label>賞味期限（任意）</label>
...
async function changeFoodQuantity(id, delta, appState) {
    ...
    if (item) {
        // quantity が null / undefined / 空文字 の場合は 0 として扱う
        const currentQty = (item.quantity != null && !isNaN(item.quantity)) ? Number(item.quantity) : 0;
...
function formatDateLocal(dateVal) {
    if (!dateVal) return '';
    // すでに YYYY/MM/DD 形式の文字列であればそのまま返す
